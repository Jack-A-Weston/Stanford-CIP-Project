...
def welcome():
    ...
    desire = input("Enter 'L' to leave a new message. Enter 'R' to retrieve message: ")
    while desire != 'L' and desire != 'R':
        desire = input("Enter 'L' or leave a new message. Enter 'R' to retrieve message: ")
    if desire == 'L':
        return True
    else:
        return False
...
def new_message(codex,count):
    if count > 0:
        ...
        if decision == 'Y':
            entry = input("Please give me your message. Note: Do not use numbers: ")
...
def email_verify(security_code):
    check = 'no'
    while check == 'no':
        email_address = str(input("Enter your email address for ID confirmation: "))
        sender_email = "cipsecretagency@gmail.com"
        receiver_email = email_address
        subject = "Agent Identity Confirmation"
        text = "Good Day: Your confirmation code is " + str(security_code) + "!"
        message = 'Subject: {}\n\n{}'.format(subject, text)
        port = 465  # For SSL
        ...
        verification = int(input("Please enter only the numbers in your confirmation code: "))
...
def set_up():
    ...
    if x == 'Y':
        ...
        while doublecheck != 'Y' and doublecheck != 'N':
            print(doublecheck)
            doublecheck = input("Not a valid input! Please try again. Enter Y or N: ")
            print(doublecheck)
        if doublecheck == 'Y':
            print("OK")
            password = ' '
